...
from money_utils.schema.fee_util import (
    DetectMethod,
    PayInterval,
    common_fee_amounts,
    fee_amounts,
...
from money_utils.schema.transaction import Transaction, TxType
...
def choose_member(tx: Transaction, members: List[Member]):
    ...
    for member in members:
        ...
        score = fuzz.token_set_ratio(member_tokens, tx_tokens) + fuzz.token_set_ratio(
            member_tokens, tx.purpose
        )
        scores[member] = score
...
def analyze_member(member, today, session):
    ...
    logging.info(
        " === Member %d %s," % (member.id, member.name)
    ...
    for first_day, next_month in util.months(from_date, today):
        ...
        if month_sum == 0:
            ...
            if member.last_fee != month_sum and not member_txs_additional(
                session, member, txs
            ):
                unchanged_months = 0
...
def analyze_fees(member, today, session: Session, all_txs: Query):
    txs_without_entry: List[Transaction] = (
        all_txs.outerjoin(FeeEntry, FeeEntry.tx_id == Transaction.tx_id)
...
def split_fee_months(entry, session, num_months, pay_interval, tx):
    ...
    for m in range(num_months):
        ...
        logging.info(
            "fee entry split_fee_months: %s (%.2f) %s", m_entry.month, m_entry.fee, tx
        )
        yield m_entry
...
def month_command(tx):
    try:
        ...
        if not months:
            return None
        for d in months:
            if d > tx.date + relativedelta(years=2) or d < tx.date - relativedelta(
                years=1
            ):
                logging.warning(
                    "possible month command ignored, out of range (-2 years or +1 months): %s",
                    tx,
                )
                return None
        return months
...
def month_command_ymd(text):
    ...
    if matches:
        for match in matches:
            start = date(
                int(match.group("year_start")), int(match.group("month_start")), 1
            )
    ...
    matches = list(
        re.finditer(
            r"(?:^|\s)(?P<month>%s) ?(?P<year>\d{4})(?=$|\s)" % month_names_or, text
        )
    )
    if matches:
        for match in matches:
