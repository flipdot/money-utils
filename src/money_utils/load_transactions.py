...
from money_utils import db, utils, hbci_client
from money_utils.schema.status import Status, LAST_LOAD
...
def get_transactions(force=False, tan_callback=hbci_client.ask_for_tan):
    with db.tx() as session:
        last_load: Status = session.query(Status).filter_by(key=LAST_LOAD).first()
        if last_load:
...
def load_transactions(session, last_load: Status, tan_callback):
    last_transaction: Transaction = (
        _query_transactions(session).order_by(desc(Transaction.entry_date)).first()
    )
...
def load_chunk(
    session: Session, fetch_from: date, fetch_to: date, now: date, tan_callback
):
    global error
    ...
    with hbci_client.get_account(tan_callback) as (conn, acc):
        logging.info("in load chunk context")
