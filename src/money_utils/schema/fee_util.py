...
month_regex_ymd_range = re.compile(
    space_or_start + iso_ym("_start") + r"\s-\s" + iso_ym("_end") + space_or_end
)

month_regex_ymd = re.compile(space_or_start + iso_ym("") + space_or_end)
month_regex_ym = re.compile(
    space_or_start + d("month", 2) + r"/" + d("year", 4) + space_or_end
)
