...
from money_utils.schema.fee_util import PayInterval
...
class Member(Base):
    ...
    txs = relationship(
        "Transaction", order_by="Transaction.date", back_populates="member"
    )
    fee_entries = relationship(
        "FeeEntry", order_by="FeeEntry.month", back_populates="member"
    )
