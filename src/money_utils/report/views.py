...
from .get_recharges import get_recharges
from .models import Transaction, FeeEntry

from .members import main as members_main
...
def members_per_month(request: HttpRequest):
    ...
    member_fees = (
        FeeEntry.objects.values("month").order_by("month").annotate(fee=Avg("fee"))
    )
    ...
    p1.extra_y_ranges = {
        "fee_range": Range1d(start=0, end=df_fees["fee"].astype(float).max() + 5)
    }
...
def member_report_view(request: HttpRequest):
    ...
    output = StringIO()
    members_main([], to=output, months=36)
