...
# from money_utils import get_config
...
class FeeEntry(models.Model):
    ...
    pk = UniqueConstraint(
        fields=["member", "month"], name="fee_entry_member_id_month_966eb854_uniq"
    )
    ...
    def __str__(self):
        return (
            f"Fee {self.month:%Y-%m} {self.member.id:3} "
...
class Member(models.Model):
    ...
    def __str__(self):
        return " ".join(
            [self.first_name, self.last_name, "(%s)" % self.nick if self.nick else ""]
        )
...
class Transaction(models.Model):
    ...
    member = models.ForeignKey(
        Member, on_delete=models.DO_NOTHING, blank=True, null=True
    )
    ...
    frst_one_off_recc = models.TextField(
        db_column="FRST_ONE_OFF_RECC", blank=True, null=True
    )  # Field name made lowercase.
    old_sepa_ci = models.TextField(
        db_column="old_SEPA_CI", blank=True, null=True
    )  # Field name made lowercase.
    old_sepa_additional_position_reference = models.TextField(
        db_column="old_SEPA_additional_position_reference", blank=True, null=True
    )  # Field name made lowercase.
    ...
    def __str__(self) -> str:
        return "Tx[from: '{sender}', purpose: '{purpose}', amount: {amount:.2f}, date: {date}]".format(
            sender=self.applicant_name,
